...
import com.ruikao.server.security.IpRateLimitService;
...
public class StudentAuthController {
    ...
    public Result<WxLoginVO> wxLogin(@RequestBody @Valid WxLoginDTO wxLoginDTO, HttpServletRequest request) {
        // 匿名接口按 IP 限流，防脚本刷微信 code 兑换
        ipRateLimitService.check("wx-login", request);
        // 不打印微信 code（短时凭证，避免敏感信息落日志）
        log.info("微信登录");
     ...
     */
    @PostMapping("/bind-wx")
    public Result<WxLoginVO> bindWx(@RequestBody @Valid WxBindDTO wxBindDTO, HttpServletRequest request) {
        // 匿名接口按 IP 限流，防脚本刷学号密码撞库
        ipRateLimitService.check("bind-wx", request);
