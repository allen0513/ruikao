...
import com.ruikao.pojo.dto.ExamAssignByClassDTO;
...
public class ExamController {
     ...
     * 按班级批量分配考试（新增学生自动去重）
     */
    @OperLog(module = "考试管理", type = "分配", description = "按班级分配考试:{#assignByClassDTO.examId}")
    @PostMapping("/assign-by-class")
    public Result<String> assignStudentsByClass(@RequestBody @Valid ExamAssignByClassDTO assignByClassDTO) {
        log.info("按班级分配考试, examId={}, classIds={}",
                assignByClassDTO.getExamId(), assignByClassDTO.getClassIds());
        examService.assignStudentsByClass(assignByClassDTO.getExamId(), assignByClassDTO.getClassIds());
        return Result.success();
    }
