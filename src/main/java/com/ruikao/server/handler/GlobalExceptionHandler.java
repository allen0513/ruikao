...
public class GlobalExceptionHandler {
    ...
    /** 业务异常（含子类：账号不存在、密码错误等），统一返回 HTTP 200 + body code=0 的约定式响应 */
    @ExceptionHandler(BaseException.class)
    public Result<String> handleBusiness(BaseException e) {
    ...
    /** 路径不存在（404），单独处理避免被兜底 500 吞掉 */
    @ExceptionHandler(NoResourceFoundException.class)
    public ResponseEntity<Result<String>> handleNotFound(NoResourceFoundException e, HttpServletRequest request) {
        log.warn("接口不存在: {} {}", request.getMethod(), request.getRequestURI());
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(Result.error("接口不存在"));
    }

    /** 请求方法不支持（405），单独处理避免被兜底 500 吞掉 */
    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    public ResponseEntity<Result<String>> handleMethodNotSupported(HttpRequestMethodNotSupportedException e, HttpServletRequest request) {
