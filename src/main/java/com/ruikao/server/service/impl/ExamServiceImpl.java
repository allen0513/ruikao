...
public class ExamServiceImpl implements ExamService {
    ...
    @Transactional
    @RedisLock(key = "assign:{#examId}")
    public void assignStudentsByClass(Long examId, List<Long> classIds) {
        // 按班级查学生（class_id IN classIds），仅统计状态正常的在籍学生
        LambdaQueryWrapper<Student> studentWrapper = new LambdaQueryWrapper<>();
        studentWrapper.in(Student::getClassId, classIds);
        studentWrapper.eq(Student::getStatus, ExamConstants.USER_STATUS_ENABLED);
        List<Student> students = studentMapper.selectList(studentWrapper);
        if (students.isEmpty()) {
            throw new BusinessException("所选班级下没有可分配的学生");
        }

        // 该考试已分配学生（去重：班级学生已分配过的不再重复写入）
        Set<Long> existing = new HashSet<>(examStudentMapper.getStudentIdsByExamId(examId));
        ...
        for (Student student : students) {
            ...
            examStudentMapper.insert(es);
            newlyAssigned.add(student.getId());
        }
        log.info("按班级分配考试完成，examId={}, 班级数={}, 新增学生数={}", examId, classIds.size(), newlyAssigned.size());

        // 新分配学生的考试列表需要刷新（按学生 keyed 驱逐）
        evictStudentExamListByExam(examId, newlyAssigned);
    }
    ...
    public List<RankVO> getExamRank(Long examId) {
        ...
        // Redis 无数据（如缓存被清空）时降级查库：已审核（成绩终态）倒序取前 10，避免排行榜空窗
        log.info("排行榜缓存为空，降级查询数据库, examId={}", examId);
        LambdaQueryWrapper<ExamRecord> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(ExamRecord::getExamId, examId)
                .eq(ExamRecord::getStatus, ExamConstants.RECORD_STATUS_AUDITED)
